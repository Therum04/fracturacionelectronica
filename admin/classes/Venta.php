...
class Venta
{
    ...
    {
        $stmt = $this->con->prepare("UPDATE ventas SET estado = 'ANULADO', sunat_estado = 'baja', descripcion_motivo = ? WHERE id = ?");
        $stmt->bind_param("si", $motivo, $id);
        if ($stmt->execute()) {
            return ['status' => 202, 'message' => 'Venta anulada correctamente'];
        } else {
            return ['status' => 303, 'message' => 'No se pudo anular la venta'];
        }
    }
}
